...
class AdminService:
    ...
    def get_all(self):
        ...
        return self.db.query("SELECT p.*, COALESCE(p1.name, o.char_id) AS name, 'superadmin' AS access_level, 0 AS sort FROM player WHERE char_id = ? "
                             "UNION "
                             "SELECT p.*, COALESCE(p1.name, o.char_id) AS name, a.access_level, "
