...
class PrivateChannelService:
    ...
    async def handle_private_channel_message(self, packet: server_packets.PrivateChannelMessage):
        if packet.private_channel_id == self.bot.char_id:
            ...
            await self.event_service.fire_event(self.PRIVATE_CHANNEL_MESSAGE_EVENT, packet)

    async def handle_private_channel_client_joined(self, packet: server_packets.PrivateChannelClientJoined):
        if packet.private_channel_id == self.bot.char_id:
            ...
            await self.event_service.fire_event(self.JOINED_PRIVATE_CHANNEL_EVENT, packet)

    async def handle_private_channel_client_left(self, packet: server_packets.PrivateChannelClientLeft):
        if packet.private_channel_id == self.bot.char_id:
            ...
            await self.event_service.fire_event(self.LEFT_PRIVATE_CHANNEL_EVENT, packet)
