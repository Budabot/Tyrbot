...
class WhatBuffsController:
    ...
    def whatbuffs_list_cmd(self, channel, sender, reply, args):
    ...
    @command(command="whatbuffs", params=[
        ...
        Any("skill")], access_level="all",
             description="Find items or nanos that buff a skill (or ability)")
    def whatbuffs_detail_cmd(self, channel, sender, reply, args):
    ...
    @command(command="whatbuffs", params=[Any("skill")], access_level="all",
             description="Find items or nanos that buff a skill (or ability)")
    def whatbuffs_skill_cmd(self, channel, sender, reply, args):
        skill_name = args[0]
    ...
    def search_for_skill(self, skill_name):
        ...
        data = self.db.query(*self.db.handle_extended_like("SELECT id, name FROM skills WHERE name <EXTENDED_LIKE=0> ?", [skill_name]))
