...
class LogController:
    ...
    def clear_logon(self, request, params):
        if self.db.query_single("SELECT logon FROM log_messages WHERE char_id = ?", [request.sender.char_id]):
            self.db.exec("UPDATE log_messages SET logon=NULL WHERE char_id = ?", [request.sender.char_id])
    ...
    def set_logon(self, request: CommandRequest, logon_message):
        if self.db.query_single("SELECT logon FROM log_messages WHERE char_id = ?", [request.sender.char_id]):
            self.db.exec("UPDATE log_messages SET logon=? WHERE char_id = ?", [logon_message, request.sender.char_id])
        else:
    ...
    def clear_logoff(self, request, params):
        if self.db.query_single("SELECT logoff FROM log_messages WHERE char_id = ?", [request.sender.char_id]):
            self.db.exec("UPDATE log_messages SET logoff=NULL WHERE char_id = ?", [request.sender.char_id])
    ...
    def set_logoff(self, request: CommandRequest, logoff_message):
        if self.db.query_single("SELECT logoff FROM log_messages WHERE char_id = ?", [request.sender.char_id]):
            self.db.exec("UPDATE log_messages SET logoff=? WHERE char_id = ?", [logoff_message, request.sender.char_id])
        else:
